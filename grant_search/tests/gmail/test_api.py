...
from grant_search.gmail.api import (
    get_service,
...
class GmailAPITestCase(unittest.TestCase):
    @patch("grant_search.gmail.api.build")
    def test_get_service(self, mock_build):
        credentials = "dummy_credentials"
        service = get_service(credentials)
        mock_build.assert_called_once_with(
            "gmail", "v1", credentials=credentials, cache_discovery=True
        )
    ...
    def test_list_thread_ids_by_query(self):
        ...
        message_ids = list_thread_ids_by_query(
            user.get_google_credentials(),
            f"after: {after_time.strftime('%Y/%m/%d')}",
            20,
        )
        self.assertIsNotNone(message_ids)
