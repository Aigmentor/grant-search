...
from grant_search.bin.encrypt_secrets import decrypt_file
from grant_search.common import DECRYPT_KEY, MODE_ENUM, get_mode
...
def get_oauth_redirect_url():
    if get_mode() == MODE_ENUM.LOCAL:
        ...
        return "https://grant_search-abc7a98eaa1f.herokuapp.com/app/oauth_redirect"
...
def get_flow(state=None):
    ...
    return google_auth_oauthlib.flow.Flow.from_client_secrets_file(
        get_client_secret_file(), scopes=SCOPES, state=state
    )
...
def oauth_handle_redirect() -> bool:
    ...
    if response_state != state:
        logging.info(
            f"skipping auth due to state mismsatch:  {state} != {response_state}"
        )
    flow = get_flow(state)
