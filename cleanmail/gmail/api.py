...
def get_message_by_id_on_thread(credentials, message_id: str) -> dict:
    ...
    return (message_id, mime_msg) if mime_msg is not None else None

def _get_latest_message_by_gthread_id(service, gthread_id: str) -> Optional[dict]:
    try:
        gthread_data = service.users().threads().get(userId='me', id=gthread_id).execute()
...
def get_latest_message_by_gthread_id_on_thread(credentials, gthread_id: str) -> Optional[Tuple[str, dict]]:
    # logging.info(f"Getting latest message on thread {gthread_id}")
    get_local_service(credentials)
    return _get_latest_message_by_gthread_id(thread_local.service, gthread_id)
...
def list_messages_by_message_id(credentials, messages: list, max_items: int) -> dict:
    ...
    if not messages:
        ...
        with ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
            results = executor.map(lambda msg: get_message_by_id_on_thread(credentials, msg), [x['id'] for x in messages])
...
def list_messages_by_gthread_id(credentials: Credentials, gthreads: list, max_items: int) -> dict:
    resolved_messages = {}
    if not gthreads:
        ...
        by_id = {}
        with ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
            results = executor.map(lambda gthread_id: get_latest_message_by_gthread_id_on_thread(credentials, gthread_id), [x['id'] for x in gthreads])
        results = [r for r in results if r is not None]
        by_id = dict(results)
        logging.info(f"Resolved {len(by_id)} messages from {len(gthreads)} threads")
        # logging.info(f"by_id keys: {by_id.keys()}")
        # Now add them to the hash in the order of the original list
        for gthread in gthreads:
            if gthread['id'] in by_id:
                resolved_messages[gthread['id']] = by_id[gthread['id']]
            else:
                resolved_messages[gthread['id']] = gthread
                logging.info('gthread == msg_id: %s', gthread['id'])
        
        return resolved_messages

def list_message_ids_by_query(credentials: Credentials, query: str, max: int = 30) -> list[str]:
    service = get_service(credentials=credentials)
    return page_all_items(
        lambda next_page_token: service.users().messages().list(pageToken=next_page_token, userId='me', q=query).execute(),
        'messages', max)
...
def list_messages_since_history_id(credentials: Credentials, start_history_id: str) -> Tuple[str, dict, list]:
    service = get_service(credentials=credentials)

    changes = page_all_items(
        lambda next_page_token: service.users().history().list(
            userId='me', 
    ...
    logging.info(f"Found {len(messages)} new emails and {len(removed_messages)} removed emails from {start_history_id} to {latest_history_id}")
    return latest_history_id, list_messages_by_message_id(credentials, messages, len(messages)), removed_messages
...
def list_thread_messages_by_query(credentials: Credentials, query: str, max: int = 30) -> dict:
    service = get_service(credentials=credentials)

    gthreads = page_all_items(          
        lambda next_page_token: service.users().threads().list(pageToken=next_page_token, userId='me', q=query).execute(),
        'threads', max)
...
def draft_reply(credentials: Credentials, user_id: str, gthread_id: str, body: str): 
    ...
    try:
        _, original_msg = get_latest_message_by_gthread_id_on_thread(credentials, gthread_id)
    except Exception as e:
...
def create_draft(credentials: Credentials, user_id: str, to_list: list[str], subject: str, body:str) -> str:
...
def archive_email(credentials: Credentials, user_id: str, email_id: str) -> bool:
    ...
    try:
        response = service.users().messages().modify(userId=user_id, id=email_id, body=commands).execute()
